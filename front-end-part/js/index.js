...
function showBlog(){
    ...
    for(var i= 0,len = button.length;i<len;i++){
        button[i].onclick = function(){
            $.post("xxx.asp",{blogId:this.blogId},function(data){  //申请写这篇博文的好友的信息，发送博文的id，接受一个json，json格式如下
//data:json
//json[0]:{nickname:}
...
function story(){
    ...
    var handler1 = function(){
        ...
        first.innerHTML = '<span></span><p>'+blog.allBlog[count].title+'</p><p>'+blog.allBlog[count].passage+'</p><button>More</button><span><img src="../img/icon_praise.png" alt=""/>133 <img src="../img/icon_comment.png" alt=""/>133 </span>';
        var first_button = first.getElementsByTagName("button");
        first_button.setAttribute("blogId",blog.allBlog[count].id);
        count++;
...
//json[3]:{myBlog:[{title:,passage:},{title:,passage:}]}
    ...
    var handler2 = function(){
        ...
        last.innerHTML = '<span></span><p>'+blog.allBlog[count].title+'</p><p>'+blog.allBlog[count].passage+'</p><button>More</button><span><img src="../img/icon_praise.png" alt=""/>133 <img src="../img/icon_comment.png" alt=""/>133 </span>';
        var last_button = last.getElementsByTagName("button");
        last_button.setAttribute("blog",blog.allBlog[count].id);
        count++;
        //i,m working here
...
function photo(){
    ...
    for(var i=0;i<7;i++){
        ...
        m[i].style.backgroundImage = 'url("../img/JPEG/'+random+'.jpg")';
    }
    var handler1 = function(){
        m[2].style.left = "3.3rem";
        ...
        first.setAttribute("id","a0");
        first.innerHTML = '<span></span> <p>昵称昵称</p> <span> <img src="../img/icon_praise.png" alt=""/>133 <img src="../img/icon_comment.png" alt=""/>133 </span>';
        random = Math.ceil(Math.random()*7%7);
        first.style.backgroundImage = 'url("../img/JPEG/'+random+'.jpg")';
        contain.insertBefore(first,m[0]);
        m[7].parentNode.removeChild(m[7]);
        for(var i=0;i<7;i++){
            m[i].setAttribute("id",m[i].id[0]+(+m[i].id[1]+1))
    ...
    var handler2 = function(){
        ...
        var contain = document.getElementById("photo");
        last.setAttribute("id","a8");
        last.innerHTML = '<span></span> <p>昵称昵称</p> <span> <img src="../img/icon_praise.png" alt=""/>133 <img src="../img/icon_comment.png" alt=""/>133 </span>';
        random = Math.ceil(Math.random()*7%7);
        last.style.backgroundImage = 'url("../img/JPEG/'+random+'.jpg")';
        contain.appendChild(last);
        m[0].parentNode.removeChild(m[0]);
        for(var i=0;i<7;i++){
            m[i].setAttribute("id",m[i].id[0]+(+m[i].id[1]-1))
...
function register(){
    ...
    $.get("register.php",function(data){  //页面加载完获得用户的登录天数
        if(data>6){
...
function userMassage(){
    $.get("thisUser.php",function(data){  //页面加载完获取用户个人信息（不包括登录天数）
        userArr = JSON.parse();
        ...
        document.getElementById("story1").innerHTML = userArr[3].myBlog[1].passage;
        document.getElementById("story2").innerHTML = userArr[3].myBlog[2].passage;
    })
}
//json[0]:{nickname:}
//json[1]:{sidebarImg:[src1,src2]}
